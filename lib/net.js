...
var NetMonitor = GObject.registerClass({
    ...
    _init(configHandler) {
        ...
        for (let i = 0; i < netlist.number; i++) {
            ...
            // Ignore loopback, virtual, and container bridge devices
            if (dev === 'lo' || dev.startsWith('vnet') || dev.startsWith('veth') || dev.startsWith('virbr') || dev.startsWith('docker') || dev.startsWith('br-')) {
                continue;
            }
